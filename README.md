...
## X2 collection extension

This repository adds an X2-hand pipeline for side-conditioned, 1--5 finger
mesh-grasp generation and Isaac Sim/PhysX v7 validation.  The formal collector
targets exactly 5000 audited valid records across 12 deterministic primitives
and 30 selected general meshes, with balanced front/back and finger-count
quotas plus complementary non-overlapping finger masks.

Start with:

- [X2 mesh-grasp generator](docs/x2_mesh_grasp_generator.md)
- [PhysX v7 validation protocol](docs/x2_physx_grasp_validation.md)
- [Formal dataset contract](docs/x2_primitive_dataset.md)
- [Collection and recovery runbook](docs/x2_collection_runbook.md)
- [Experiment log](docs/x2_experiment_log.md)

The public repository contains source code, tests, configuration, operational
documentation, and sanitized sample visualizations.  It intentionally excludes
raw/valid/failed records, simulator caches, checkpoints, the 88 general-object
meshes, and X2 USD/geometry assets.  To run the pipeline, provide the locally
licensed X2 asset at `x2_mujoco/x2_keypoints.usda` (including its payloads) and
the general meshes under `data/meshdata/<object_id>/coacd/decomposed.obj`.
Generated raw optimizer poses are not valid data until they pass the documented
six-direction PhysX protocol.

This extension remains subject to the upstream attribution and CC BY-NC 4.0
terms stated below; no omitted robot or object asset license is granted here.
